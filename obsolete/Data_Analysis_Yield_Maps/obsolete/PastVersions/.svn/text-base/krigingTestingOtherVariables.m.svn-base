     ...
     red_ground_counts = [red_not_thinned_ground_counts; red_thinned_ground_counts];
     ...
     apple_types_ground_counts{1, 4} = red_ground_counts;
     ...
     apple_types_pc_counts{1, 4} = red_pc_counts;
     ...
     for i = 1:types_of_apples
         ...
         newx = zeros(numel(resized_image_pc), 1);
         newy = zeros(numel(resized_image_pc), 1);
         newz = zeros(numel(resized_image_pc), 1);
         ...
         for j = 1:rows_pc_image
            for k = 1:columns_pc_image
                ...
                newx(current_place) = j*(scaling_factor) - scaling_factor/2;
                newy(current_place) = k*(scaling_factor) - scaling_factor/2;
                newz(current_place) = counted_map_pc(k, j);
            end
         end

         v = variogram([newx newy], newz,'plotit',false, 'nrbins', 10, 'type', 'cloud1');
         v
