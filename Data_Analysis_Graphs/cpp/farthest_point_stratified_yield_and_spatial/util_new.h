...
inline double sqr_dist_unit_torus(const Point_2_new &p0, const Point_2_new &p1) {
...
inline Point_2_new wrap_unit_torus(const Point_2_new &p) {
    double x = p.x() < 0 ? p.x() + 1.0 : (p.x() >= 1.0 ? p.x() - 1.0 : p.x());
    double y = p.y() < 0 ? p.y() + 1.0 : (p.y() >= 1.0 ? p.y() - 1.0 : p.y());
    return Point_2_new(x, y);
}

inline bool is_in_unit_torus(const Point_2_new &p) {
    return 0.0 <= p.x() && p.x() < 1.0 &&
           0.0 <= p.y() && p.y() < 1.0;
}

inline bool is_in_rect(const Point_2_new &p,
                       const Point_2_new &clip0, const Point_2_new &clip1) {
