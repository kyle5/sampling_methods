...
%Kyle: This is where the difference in z is calculated for every pair of
%points calculate squared difference between values of coordinate pairs
...
switch params.type
    case {'default','gamma'}
        ...
        % ixedge is an index matrix, which represents the indecies within lam
        % that should be used for each the calculation of the values of
        % for each bin
        ...
        if params.anisotropy
            ...
            %Kyle: For each 'bin' of points that is within
            %the 'bin' distances, get the distance between the point pairs
            %and then do the equation on each of the points.

            
            % fvar gets the average difference in values for all of the
            % point pairs that are within a bin
            S.val = accumarray(ixedge,lam,[numel(edges) 1],fvar,nan);
            %Kyle: Not sure about below... Counts the number of pairs of
            %points that are within the range of one another for
            %the calculation?
            S.num = accumarray(ixedge,ones(size(lam)),[numel(edges) 1],@sum,nan);
        end
        
        %S.distance seems to be array of the midway points of each bin:
        S.distance = (edges(1:end-1)+tol/2)';
        
        
        % The end value needs to be eliminated because there are 1:end-1
        % distance values and 1:end 'val' values.
